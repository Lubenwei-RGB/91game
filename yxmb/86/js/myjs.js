...
let need_money = [1000,3500,5300,7000,8500,9100,10600,12100,13900,15500,17580,26000];
	...
	setInterval(function(){
		...
		if(leave_time == 0)
		{
			if(dollar < need_money[now_floor]) 
			{
				...
				Guoducontext.fillText("你没把林强挖爽！"  ,415,390);
				leave_time = -9999;
			}
			else{
				$("messshow").height = $("messshow").height;
		...
		else if(hook_state == 1)
		{
			...
			if(exist[i])
			{
				...
				if(mi< radius[i] * radius[i]*1)
				{
					...
					$("hook_back").src = "music/untitled.ogg";
					$("hook_back").play();
					musicre = 0;
					break;
				}
...
document.addEventListener('mousedown', function(e) {
	if(e.button === 0 && hook_state === 0) { // 0 表示鼠标左键
		catch_material = 0;
		$("hook_out").src = "music/wj2.ogg";
		$("hook_out").play();
		hook_state = 1;
